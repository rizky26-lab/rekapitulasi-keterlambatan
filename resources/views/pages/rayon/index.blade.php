...
<div class="card">
    <div class="card-body">
        <div class="d-flex justify-content-lg-end mb-3">
            <a class="btn btn-success" href="{{ route('rayon.create') }}">Tambah Rayon</a>
        </div>
            ...
            <table class="table table-hover table-bordered" id="my_table">
                <thead class="bg-light">
                    <tr>
                        ...
                        <th class="text-nowrap">Pembimbing Siswa</th>
                ...
                <tbody>
                    @foreach ($rayon as $item)
                        <tr>
                            ...
                            <td class="text-nowrap">{{ $item->rayon }}</td>
                            <td class="text-nowrap">{{ $item->user->name }}</td>
                            <td class="text-nowrap">
                                <div class="d-flex">
                                    <a href="{{ route('rayon.edit',$item->id) }}" class="btn btn-warning me-2">Edit</a>
                                    <form action="{{ route('rayon.destroy',$item->id) }}" method="post" id="delete_form{{ $item->id }}">
                                        @csrf
                                        @method('delete')
