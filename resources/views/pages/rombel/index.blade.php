...
<div class="card">
    <div class="card-body">
        <div class="d-flex justify-content-lg-end mb-3">
            <a class="btn btn-success" href="{{ route('rombel.create') }}">Tambah Rombel</a>
        </div>
                ...
                <tbody>
                    @foreach ($rombel as $item)
                        <tr>
                            ...
                            <td class="text-nowrap">{{ $item->rombel }}</td>
                            <td class="text-nowrap">
                                <div class="d-flex">
                                    <a href="{{ route('rombel.edit',$item->id) }}" class="btn btn-warning me-2">Edit</a>
                                    <form action="{{ route('rombel.destroy',$item->id) }}" method="post" id="delete_form{{ $item->id }}">
                                        @csrf
                                        @method('delete')
